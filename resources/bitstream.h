...
static inline void bs_init(bitstream_t *bs, const uint8_t *data, size_t length) {
    bs->cur = data;
    bs->end = data + length;
    bs->bit = 0;
}

static inline bool bs_has_bytes(bitstream_t *bs, size_t bytes) {
    return bs->end - bs->cur >= bytes;
}
...
static inline uint8_t bs_read_from_inside_byte(bitstream_t *bs, unsigned width) {
...
static inline uint8_t bs_read_bits_to_u8(bitstream_t *bs, unsigned width) {
    uint8_t result;
...
static inline uint8_t bs_read_1_byte(bitstream_t *bs) {
    return *bs->cur++;
}

static inline uint8_t bs_read_high_order_bits(bitstream_t *bs, unsigned width) {
    bs->bit = width;
    return *bs->cur >> (8 - width);
}

static inline uint16_t bs_read_bits_to_u16(bitstream_t *bs, unsigned width) {
    uint16_t result;
...
static inline uint32_t bs_read_bits_to_u32(bitstream_t *bs, unsigned width) {
    uint32_t result;
...
static inline uint32_t bs_read_bits_to_u64(bitstream_t *bs, unsigned width) {
    uint64_t result;
